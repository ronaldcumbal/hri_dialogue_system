...
class Face:
    ...
    def initialise_publishers(self):
        ...
        #modified# ns = f'/humans/faces/{self.id}'
        ns = f'/faces/{self.id}'
        self.roi_pub = self.node.create_publisher(NormalizedRegionOfInterest2D, ns + '/roi', 1)
        #removed# self.cropped_pub = self.node.create_publisher(Image, ns + '/cropped', 1)
        #removed# self.aligned_pub = self.node.create_publisher(Image, ns + '/aligned', 1)
    ...
    def publish(self, src_image: RGBMat, image_msg_header: Header):
        ...
        #removed# self.publish_cropped_face(src_image, image_msg_header)
        #removed# self.publish_aligned_face(src_image, image_msg_header)
    ...
    def __del__(self):
        ...
        #removed# self.node.destroy_publisher(self.cropped_pub)
        #removed# self.node.destroy_publisher(self.aligned_pub)
...
class NodeFaceDetect(Node):
    ...
    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        #modified# self.faces_pub = self.create_publisher(IdsList, '/humans/faces/tracked', 1)
        self.faces_pub = self.create_publisher(IdsList, '/faces/tracked', 1)
